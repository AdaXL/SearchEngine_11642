...
public class QryParser {
    ...
    private static Qry createOperator(String operatorName) {
        ...
        // Handle NEAR/n operator and get the value of n.
        if(operatorNameLowerCase.contains("/")) {
            String[] operatorIop = operatorNameLowerCase.split("[/]");
            operatorNameLowerCase = operatorIop[0];
            operatorDistance = Integer.valueOf(operatorIop[1]);
        }
        ...
        switch (operatorNameLowerCase) {
            ...
            // Handle NEAR operator and AND operator.
            case "#and":
    ...
    private static Qry[] createTerms(String token) throws IOException {
        ...
        for (int j = 0; j < t.length; j++) {
            ...
            //System.out.println("Now processing terms: " + terms[j]);
        }
    ...
    private static Qry parseString(String queryString)
        ...
        while (queryString.length() > 0) {
            ...
            // Check if current queryTree is WAND or WSUM, for handling weights later.

            if (queryTree instanceof QrySopWAnd || queryTree instanceof QrySopWSum) {

                // Get the weight of cur Qry.
                ...
                if (queryString.charAt(0) == '#') {      // Subquery
                    ...
                    // Assign weight to cur Qry.

                    qargs[0].weight = weight;

                } else {        // Term
                    p = popTerm(queryString);
                    qargs = createTerms(p.getPopped());

                    // In case a term gets tokenized into multiple terms, "near-death" to "near" and "death".
                    for (int i = 0; i < qargs.length; i ++) {
                        qargs[i].weight = weight;
                    }
                }
            } else {    // If current queryTree is not WAND or WSUM, which means it doesn't contain weight value.
                if (queryString.charAt(0) == '#') {    // Subquery
                    p = popSubquery(queryString);
                    qargs = new Qry[1];
                    qargs[0] = parseString(p.getPopped());
        ...
        //System.out.println("Ready to return: " + queryTree);
